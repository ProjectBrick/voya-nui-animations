...
async function * files() {
	const reg = /^[^.]+\.(swf|xml|txt)$/i;
	for (let i = 1; i <= 6; i++) {
		const b = `original/files/2006`;
		const ignore = new Set([
			`ep${i}/detect.swf`
		]);
		const dirs = [`ep${i}`];
		if (i === 6) {
			dirs.push(`ep${i}/scenes`);
		}
		for (const d of dirs) {
			for (const n of (await fse.readdir(`${b}/${d}`)).sort()) {
				const f = `${d}/${n}`;
				if (!reg.test(n) || ignore.has(f)) {
					continue;
				}
				yield [f, await fse.readFile(`${b}/${f}`)];
			}
		}
	}
	for (const f of [
		'voyanuianimations.swf'
	]) {
